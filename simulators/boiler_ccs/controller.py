...
InternalHook = Optional[Callable[[str, Dict[str, float]], Optional[Dict[str, float]]]]
...
def _override_output(
    ...
    *,
    ...
    if not isinstance(overrides, dict):
        raise TypeError("internal hook must return dict or None")
...
class LoadInstructionGenerator:
    ...
    def __init__(self, Ts: float, rate_limit: float = 5.0, initial_output: float = 349.0):
        ...
        self.last_debug: Dict[str, Any] = {}
        self.last_output: Dict[str, float] = {"loadgen_rate_output": float(initial_output)}


    def update(self, actual_load: float, hook: InternalHook = None) -> float:

        #
        out = self.rate_limiter.update(float(actual_load))
        out = _override_output(
            hook,
            phase="controller_loadgen_rate_output",
            field="loadgen_rate_output",
            value=out,
        )
        self.rate_limiter.prev_output = float(out)

        self.last_debug = self.rate_limiter.last_debug
        self.last_output = {"loadgen_rate_output": float(out)}
    ...
    def reset(self, initial_output: float = 349.0) -> None:
        ...
        self.last_output = {"loadgen_rate_output": float(initial_output)}
...
class MainSteamPressureController:
    ...
    def update(
        ...
        load_output: float,
        pressure_bias: float,
        unit_pressure_sp: float,
        hook: InternalHook = None,
    ) -> Tuple[float, float]:
        ...
        h1 = _override_output(
            hook,
            phase="controller_steam_hfop1_output",
            field="steam_hfop1_output",
            value=h1,
        )
        self.hfop1.av_0 = float(h1)

        h2 = self.hfop2.update(h1)
        h2 = _override_output(
            hook,
            phase="controller_steam_hfop2_output",
            field="steam_hfop2_output",
            value=h2,
        )
        self.hfop2.av_0 = float(h2)

        h3 = self.hfop3.update(h2)
        h3 = _override_output(
            hook,
            phase="controller_steam_hfop3_output",
            field="steam_hfop3_output",
            value=h3,
        )
        self.hfop3.av_0 = float(h3)

        h4 = self.hfop4.update(h3)
        h4 = _override_output(
            hook,
            phase="controller_steam_hfop4_output",
            field="steam_hfop4_output",
            value=h4,
        )
        self.hfop4.av_0 = float(h4)

        #
        hslim = self.rate.update(h4)
        hslim = _override_output(
            hook,
            phase="controller_steam_rate_output",
            field="steam_rate_output",
            value=hslim,
        )
        self.rate.prev_output = float(hslim)
        ...
        self.last_output = {
            "steam_hfop1_output": float(h1),
            "steam_hfop2_output": float(h2),
            "steam_hfop3_output": float(h3),
            "steam_hfop4_output": float(h4),
            "steam_rate_output": float(hslim),
        }
    ...
    def reset(self, initial_output: float = 23.7) -> None:
        ...
        self.last_output = {
            "steam_hfop1_output": float(initial_output),
            "steam_hfop2_output": float(initial_output),
            "steam_hfop3_output": float(initial_output),
            "steam_hfop4_output": float(initial_output),
            "steam_rate_output": float(initial_output),
        }
...
class BoilerMasterController:
    ...
    def update(
        ...
        steam_setpoint: float,
        hook: InternalHook = None,
    ) -> float:

        #
        pv_filtered = self.hfop1.update(float(main_steam_pressure))
        pv_filtered = _override_output(
            hook,
            phase="controller_boiler_pv_filtered",
            field="boiler_pv_filtered",
            value=pv_filtered,
        )
        self.hfop1.av_0 = float(pv_filtered)

        boiler_cmd = self.HSV.update(PV=pv_filtered, SP=float(steam_setpoint))
        boiler_cmd = _override_output(
            hook,
            phase="controller_boiler_pid_output",
            field="boiler_pid_output",
            value=boiler_cmd,
        )
        self.HSV.AV_0 = float(boiler_cmd)

        self.last_debug = self.HSV.last_debug
        self.last_output = {
            "boiler_pv_filtered": float(pv_filtered),
            "boiler_pid_output": float(boiler_cmd),
        }
    ...
    def reset(self, initial_output: float = 157.6) -> None:
        ...
        self.last_output = {
            "boiler_pv_filtered": 23.7,
            "boiler_pid_output": float(initial_output),
        }
...
class FuelController:
    ...
    def update(
        ...
        rate_out = _override_output(
            hook,
            phase="controller_fuel_rate_output",
            field="fuel_rate_output",
            value=rate_out,
        )
        self.rate.prev_output = float(rate_out)

        #
        pid_out = self.hsvpid.update(PV=float(coal_flow), SP=rate_out)
        pid_out = _override_output(
            hook,
            phase="controller_fuel_pid_output",
            field="fuel_pid_output",
            value=pid_out,
        )
        self.hsvpid.AV_0 = float(pid_out)

        fuel_cmd = self.hfop1.update(pid_out)
        fuel_cmd = _override_output(
            hook,
            phase="controller_fuel_filter_output",
            field="fuel_filter_output",
            value=fuel_cmd,
        )
        self.hfop1.av_0 = float(fuel_cmd)
        ...
        self.last_output = {
            "fuel_rate_output": float(rate_out),
            "fuel_pid_output": float(pid_out),
            "fuel_filter_output": float(fuel_cmd),
        }
    ...
    def reset(self, initial_output: float = 157.6) -> None:
        ...
        self.last_output = {
            "fuel_rate_output": float(initial_output),
            "fuel_pid_output": float(initial_output),
            "fuel_filter_output": float(initial_output),
        }
...
class WaterK:
    ...
    def __init__(self, Ts: float):
        ...
        self.last_debug: Dict[str, Any] = {}
        self.last_output: Dict[str, float] = {"waterk_filter_output": 1065.7}
    ...
    def update(
        ...
        water_sp = _override_output(
            hook,
            phase="controller_waterk_filter_output",
            field="waterk_filter_output",
            value=water_sp,
        )
        self.hfop1.av_0 = float(water_sp)
        ...
        self.last_output = {"waterk_filter_output": float(water_sp)}
    ...
    def reset(self, initial_output: float = 1065.7) -> None:
        ...
        self.hfop1.reset(initial_output)
        self.last_output = {"waterk_filter_output": float(initial_output)}
...
class FeedwaterController:
    ...
    def update(
        ...
        pid_out = _override_output(
            hook,
            phase="controller_water_pid_output",
            field="water_pid_output",
            value=pid_out,
        )
        self.pid.AV_0 = float(pid_out)

        rate_out = self.rate.update(pid_out)
        rate_out = _override_output(
            hook,
            phase="controller_water_rate_output",
            field="water_rate_output",
            value=rate_out,
        )
        self.rate.prev_output = float(rate_out)

        pump_speed = self.hfop.update(rate_out)
        pump_speed = _override_output(
            hook,
            phase="controller_water_filter_output",
            field="water_filter_output",
            value=pump_speed,
        )
        self.hfop.av_0 = float(pump_speed)
        ...
        self.last_output = {
            "water_pid_output": float(pid_out),
            "water_rate_output": float(rate_out),
            "water_filter_output": float(pump_speed),
        }
    ...
    def reset(self, initial_output: float = 2500.0) -> None:
        ...
        self.last_output = {
            "water_pid_output": float(initial_output),
            "water_rate_output": float(initial_output),
            "water_filter_output": float(initial_output),
        }
