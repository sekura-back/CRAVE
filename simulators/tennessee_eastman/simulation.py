...
"""TEP closed-loop simulation for the CRAVE Tennessee Eastman backend.
...
CANONICAL_SETPOINT_FIELDS: Dict[str, int] = {
    ...
    "separator_level_sp": 6,

    "stripper_level_sp": 7,

    "steam_flow_setpoint": 8,

    "cw_temp_setpoint": 9,

    "stripper_underflow_sp": 10,

    "separator_pressure_sp": 11,

    "reactor_feed_a_comp_sp": 12,

    "reactor_feed_d_comp_sp": 13,

    "reactor_feed_e_comp_sp": 14,

    "stripper_temp_setpoint": 15,

    "reactor_level_sp": 16,

    "reactor_temp_setpoint": 17,

    "purge_b_comp_sp": 18,

    "product_e_comp_sp": 19,

}

TRACE_CONTROLLER_OUTPUT_FIELDS: Tuple[str, ...] = (
    *(f"xmv_{idx:02d}_ctrl_output" for idx in range(1, 13)),
    ...
    "purge_override_high_output",
    "purge_override_low_output",
    "purge_normal_ctrl_output",
)

TRACE_SCALED_OUTPUT_SETPOINTS: Dict[str, int] = {
    "ctrl13_scaled_output": 2,
    "ctrl14_scaled_output": 0,
    "ctrl15_scaled_output": 1,
    "ctrl16_scaled_output": 8,
    "ctrl17_scaled_output": 3,
    "ctrl18_scaled_output": 9,
    "ctrl19_scaled_output": 5,
    "ctrl20_scaled_output": 15,
}
...
HAZARD_RULES = [
    {
        "id": "H-TEP-REACTOR-PRESSURE",
        "kind": "threshold",
        "expr": "xmeas_07 > 3000.0",
        "var": "xmeas_07",
        "threshold": 3000.0,
        "direction": "upper",
        "component": "physical",
...
ALARM_AFEED_ABS = _env_float("CRAVE_TE_ALARM_AFEED_ABS", 0.5)

ALARM_ACFEED_ABS = _env_float("CRAVE_TE_ALARM_ACFEED_ABS", 1.5)

ALARM_RECYCLE_ABS = _env_float("CRAVE_TE_ALARM_RECYCLE_ABS", 1.4)

ALARM_SEPARATOR_LEVEL_ABS = _env_float("CRAVE_TE_ALARM_SEPARATOR_LEVEL_ABS", 10.0)

ALARM_STRIPPER_LEVEL_ABS = _env_float("CRAVE_TE_ALARM_STRIPPER_LEVEL_ABS", 40.0)

ALARM_REACTOR_LEVEL_ABS = _env_float("CRAVE_TE_ALARM_REACTOR_LEVEL_ABS", 14.5)

ALARM_PURGE_ABS = _env_float("CRAVE_TE_ALARM_PURGE_ABS", 0.2)
...
class ClosedLoopSim:
    ...
    def step(
        ...
        def controller_hook(phase: str, context: Dict[str, float]) -> Optional[Dict[str, float]]:
            merged, modified = self._apply_hook(t_step, phase, context, injection_hook)
            return merged if modified else None

        # 1) Controller cycle.

        new_xmv = self.controller.calculate(
            xmeas_arr,
            xmv_prev,
            int(t_step),
        ...
        post_sp_ctx, post_sp_modified = self._apply_hook(

            t_step, "controller_setpoints", post_sp_ctx, injection_hook
        ...
        if post_sp_modified:
            ...
            for field, idx in SETPOINT_FIELDS.items():
                ...
                if field in post_sp_ctx:

                    self.controller.setpoints[idx] = float(post_sp_ctx[field])
        ...
        sp_separator_level = float(sp[6])  # XMEAS(12)

        sp_stripper_level = float(sp[7])   # XMEAS(15)

        sp_steam_flow = float(sp[8])

        sp_cw = float(sp[9])            # XMEAS(21)

        sp_stripper_temp = float(sp[15])

        sp_reactor_level = float(sp[16])  # XMEAS(8)
        ...
        meas_separator_level = float(new_state["xmeas_12"])

        meas_stripper_level = float(new_state["xmeas_15"])

        meas_cw = float(new_state["xmeas_21"])

        meas_reactor_level = float(new_state["xmeas_08"])
        ...
        alarm_thr_separator_level = ALARM_SEPARATOR_LEVEL_ABS

        alarm_thr_stripper_level = ALARM_STRIPPER_LEVEL_ABS

        alarm_thr_reactor_level = ALARM_REACTOR_LEVEL_ABS
        ...
        alarm_margins = {
            ...
            "alarm_sep_level_track_margin":

                alarm_thr_separator_level - abs(meas_separator_level - sp_separator_level),

            "alarm_stripper_level_track_margin":

                alarm_thr_stripper_level - abs(meas_stripper_level - sp_stripper_level),

            "alarm_reactor_level_track_margin":

                alarm_thr_reactor_level - abs(meas_reactor_level - sp_reactor_level),
        ...
        alarm_rule_map = {
            ...
            "alarm_d_feed_track_margin": "A-TEP-DFEED-TRACK",

            "alarm_e_feed_track_margin": "A-TEP-EFEED-TRACK",

            "alarm_a_feed_track_margin": "A-TEP-AFEED-TRACK",

            "alarm_ac_feed_track_margin": "A-TEP-ACFEED-TRACK",

            "alarm_recycle_flow_track_margin": "A-TEP-RECYCLE-FLOW-TRACK",

            "alarm_sep_level_track_margin": "A-TEP-SEP-LEVEL-TRACK",

            "alarm_stripper_level_track_margin": "A-TEP-STRIPPER-LEVEL-TRACK",

            "alarm_reactor_level_track_margin": "A-TEP-REACTOR-LEVEL-TRACK",

            "alarm_purge_rate_track_margin": "A-TEP-PURGE-RATE-TRACK",
        ...
        for field, idx in CANONICAL_SETPOINT_FIELDS.items():
            record[field] = float(self.controller.setpoints[idx])
        record["sp_ac_feed"] = float(
            self.controller.setpoints[CANONICAL_SETPOINT_FIELDS["ac_feed_sp"]]
        )

        trace_controller_outputs = dict(getattr(self.controller, "last_debug", {}))
        for idx in range(1, 13):
            field = f"xmv_{idx:02d}_ctrl_output"
            trace_controller_outputs.setdefault(field, controller_xmv[f"xmv_{idx:02d}"])
        for field, idx in TRACE_SCALED_OUTPUT_SETPOINTS.items():
            trace_controller_outputs.setdefault(field, float(self.controller.setpoints[idx]))
        trace_controller_outputs.setdefault("purge_override_high_output", 100.0)
        trace_controller_outputs.setdefault("purge_override_low_output", 0.0)
        trace_controller_outputs.setdefault(
            "purge_normal_ctrl_output",
            controller_xmv["xmv_06"],
        )
        for field in TRACE_CONTROLLER_OUTPUT_FIELDS:
            record[field] = float(trace_controller_outputs.get(field, 0.0))
