...
from src.platforms.variable_mapping import (
    add_variable_aliases_to_injection_registry,
    build_platform_variable_map,
)
...
def load_injection_registry(manifest_path: Path) -> Dict[str, Dict[str, Any]]:
    ...
    variable_map = build_platform_variable_map(root, manifest_path.parent.name)
    return add_variable_aliases_to_injection_registry(registry, variable_map)
...
def load_rules_from_manifest(manifest_path: Path) -> Dict[str, Any]:
    ...
    for ar in manifest.get("alarm_rules", []):
        ...
        alarms[rule_id] = {
            ...
            "setpoint_var": ar.get("setpoint_var", ""),
...
def _make_ramp_hold_hook(
    ...
    *,
    ...
    state: Dict[str, Optional[float]] = {
        ...
        "t0": None, "base_value": None, "terminal_effect": None,
    ...
    def hook(t_step: int, current_phase: str, context: Dict[str, float]) -> Optional[Dict[str, float]]:
        ...
        if state["base_value"] is None:
            ...
            state["base_value"] = float(context[field])



        if rel >= t_end:

            if state["terminal_effect"] is None:

                terminal_step = max(0, duration - 1)

                state["terminal_effect"] = amp_rate * terminal_step

            injected = _clamp(
                float(state["base_value"]) + float(state["terminal_effect"]),
                value_range,
            )
        ...
        injected = _clamp(float(state["base_value"]) + amp_rate * local_step, value_range)

        if rel == t_end - 1 and state["terminal_effect"] is None:

            state["terminal_effect"] = amp_rate * local_step
...
def annotate_rows(
    ...
    for idx, raw in enumerate(rows):
        ...
        alarm_ids = set(_coerce_rule_ids(row.get("alarm_rule_ids", [])))
        ...
        hazard_ids = set(_coerce_rule_ids(row.get("hazard_rule_ids", [])))
...
def simulate(
    ...
    *,
    ...
    ignore_stop = runtime_args.get("ignore_protection_predicates_for_stop", [])
    if isinstance(ignore_stop, str):
    ...
    try:
        ...
        trace, meta = sim.run(
            ...
            ignore_protection_predicates_for_stop=ignore_stop,
